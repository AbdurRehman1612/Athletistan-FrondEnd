...
const MyProfile = () => {
  ...
  const areaofinterest = useSelector(
    (state) => state.auth?.authData?.result?.areaofinterestolympics
  );
  const routehome = () => {
    history.push("/");
  };
  ...
  const getdata = (e) => {
    ...
    if (
      ...
      dispatch(updateathprofiledata(resetprofile, history));
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      dispatch(changeathletepassword(resetpass, history));
